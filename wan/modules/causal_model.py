from wan.modules.attention import attention, headkv_attention, headkv_window_attention
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if isinstance(kv_cache, HeadKVCache):
                # HeadKV path: per-head adaptive KV caching
                roped_query = causal_rope_apply(
                    q, grid_sizes, freqs, start_frame=current_start_frame).type_as(v)

                if cache_update_mode == "clean":
                    # Clean pass: update cache + standard HeadKV varlen attention (single block)
                    post_prune_rope = getattr(kv_cache, "post_prune_rope", False)
                    if post_prune_rope:
                        # pass unroped k; headkv_attention does dynamic RoPE internally
                        k_for_cache = k
                    else:
                        k_for_cache = causal_rope_apply(
                    ...
                    # Denoising pass: read-only cache + bidirectional window attention
                    roped_key = causal_rope_apply(
                        k, grid_sizes, freqs, start_frame=current_start_frame).type_as(v)

                    cache_is_empty = all(idx == 0 for idx in kv_cache.global_end_index)
                    if cache_is_empty:
                        # No historical cache yet, standard bidirectional attention
                        x = attention(roped_query, roped_key, v)
                    else:
                        # Combine per-head ragged historical KV with window KV
                        x = headkv_window_attention(
                            q=roped_query,
                            k=roped_key,
                            v=v,
                            kv_cache=kv_cache,
                ...
                # Dict-based KV cache path (original rolling forcing)
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        y = self.self_attn(
            ...
            seq_lens, grid_sizes,
            freqs, block_mask, kv_cache, current_start, cache_start, cache_update_mode=cache_update_mode)
